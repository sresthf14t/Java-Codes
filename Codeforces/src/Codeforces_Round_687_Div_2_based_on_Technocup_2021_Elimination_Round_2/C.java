...
public class C {
    ...
    public static void main(String args[]) throws IOException {
        ...
        for(int tt=1;tt<=test;tt++) {
            ...
            ans.append(solve(0,false)+"\n");
        }
        System.out.println(ans);
    }
    public static long solve(int indx,boolean strt) {
        if(indx>=n) {
            if(strt) {
                return 0;
            }
        ...
        if(!strt) {
            if((n-1-indx+1)>p) {
                ans=Math.min(ans,y+solve(indx+1,false));
            }
            ans=Math.min(ans, solve(indx+p-1,true));
        }
        else {
            ans=Math.min(ans, (arr[indx]==0?x:0)+solve(indx+k,true));
        }
