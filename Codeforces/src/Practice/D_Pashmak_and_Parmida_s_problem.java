...
public class D_Pashmak_and_Parmida_s_problem {
    ...
    public static void main(String args[]) throws IOException {
        ...
        arr=coordinate_compresion(n,arr);
    ...
    public static long solve(int n,int arr[]) {
        ...
        seg_tree=new int[4*tmp.length];
        create_seg_tree(tmp,0,0,tmp.length-1);
        
        long cnt=0;
        for(int i=0;i<n;i++) {
//            System.out.println(prefix[i]);
            update(0,0,tmp.length-1,suffix[i],-1);
            cnt+=sum(0,0,tmp.length-1,0,prefix[i]-1);
//            System.out.println(sum(0,0,tmp.length-1,prefix[i]+1,tmp.length-1));
        }
        return cnt;
    }
    public static int[] coordinate_compresion(int n,int arr[]) {
