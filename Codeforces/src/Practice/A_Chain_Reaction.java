...
public class A_Chain_Reaction {
    ...
    public static void main(String args[]) throws IOException {
        ...
        heapSort(pos,pow,n);
        System.out.println(n-solve(n,pos,pow));
    }
    public static int solve(int n,int pos[],int pow[]) {
        int dp[]=new int[n];
        dp[0]=1;
        for(int i=1;i<n;i++) {
            int target=pos[i]-pow[i];
            int indx=bin_search(pos,0,i-1,target);
            if(indx==-1) {
                dp[i]=1;
                continue;
            }
...
//        System.out.println();
    ...
    private static int bin_search(int[] arr, int l,int r, int target)  {  
        int start = l, end = r;  
