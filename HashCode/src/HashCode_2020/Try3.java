...
//        List<Integer> no_of_books_in_lib[]= new ArrayList[Lib]<>();
        ArrayList<Integer>[] no_of_books_in_lib = new ArrayList[Lib]; 
        ...
        for(int i=0;i<Lib;i++) {
            no_of_books_in_lib[i] = new ArrayList<Integer>(); 
            no_of_books_in_lib[i].add(input.nextInt());
//            no_of_books_in_lib[0][i]=input.nextInt();
            no_of_days_sign[i]=input.nextInt();
            no_of_books_to_scan[i]=input.nextInt();
            for(int j=1;j<=no_of_books_in_lib[i].get(0);j++) {
                no_of_books_in_lib[i].add(input.nextInt());
            }
        }
...
//        System.out.println();
        ...
        for(int i=0;i<Lib;i++) {
            ...
            int count_books_to_send=0,len=no_of_books_in_lib[i].get(0);
            for(int j=1;j<=len;j++) {
                boolean len=false;
                Collections.sort(no_of_books_in_lib[i], Collections.reverseOrder());
//                no_of_books_in_lib[i].sort(null);
                int book_no=no_of_books_in_lib[i].get(j);
                if(!book_is_taken[book_no] || book_is_taken[book_no]) {
                    count_books_to_send++;
