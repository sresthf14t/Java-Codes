...
//        System.exit(0);
        ...
        while(Days>0 && count_lib>0) {
            ...
            int best_lib=find_best_Lib(Lib,Days,b_score , no_of_days_sign, no_of_books_to_scan, books_in_lib,no_of_books_in_lib,left_rng,right_rng);
...
//               System.out.println(books);
    ...
    public static int find_best_Lib(int Lib,int Days, int[] b_score ,int[] no_of_days_sign, int[] no_of_books_to_scan, ArrayList<Integer>[] books_in_lib,int[] no_of_books_in_lib,int left_rng,int right_rng) {
        int best_lib=-1,best_score=0;
        for(int i=0;i<Lib;i++) {
            if(Lib_is_taken[i]) {
                continue;
            }
            if(no_of_days_sign[i]>=left_rng && no_of_days_sign[i]<=right_rng);
            else {
                continue;
            }
