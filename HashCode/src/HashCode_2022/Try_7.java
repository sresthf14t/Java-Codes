...
//            
            ...
            int index_of_pending_project = index_of_pending_project(project_done, 0, i, duration_of_project, deadline_of_project, score_of_project, projects_on_standby);
            ...
            for(int j=0;j<skills_name_required_for_project[index_of_pending_project].size();j++) {
                ...
                if(has_a_mentor(skills_name_required_for_project[index_of_pending_project].get(j), skills_level_required_for_project[index_of_pending_project].get(j), index_of_contributors, skills_name, skills_level)) {
                    contributor_index = index_of_free_contributor_with_reqiurnments(skills_name_required_for_project[index_of_pending_project].get(j), skills_level_required_for_project[index_of_pending_project].get(j)-1, n_contributors, skills_name, skills_level, contributor_busy_till, power_level, i, index_of_contributors);
                }
                else {
                    contributor_index = index_of_free_contributor_with_reqiurnments(skills_name_required_for_project[index_of_pending_project].get(j), skills_level_required_for_project[index_of_pending_project].get(j), n_contributors, skills_name, skills_level, contributor_busy_till, power_level, i, index_of_contributors);
                }
...
//                }
    ...
    public static int index_of_pending_project_nearest_deadline(boolean project_done[], int start_index, int date, int duration_of_project[], int deadline_of_project[], int score_of_project[], HashSet<Integer> projects_on_standby) {
        
        int max_score_possible=0;
        int index=-1;
        int days_in_hand=99999999;
        ...
        for(int i=start_index;i<project_done.length;i++) {
            ...
            if( ((date+duration_of_project[i]-1) -deadline_of_project[i]) < days_in_hand) {
                max_score_possible = project_score;
                days_in_hand = (date+duration_of_project[i]-1) -deadline_of_project[i];
                index = i;
            }
            
            
            else if( ((date+duration_of_project[i]-1) - deadline_of_project[i]) == days_in_hand && project_score > max_score_possible) {
                max_score_possible = project_score;
                days_in_hand = (date+duration_of_project[i]-1) -deadline_of_project[i];
                index = i;
            }
    ...
    public static boolean has_a_mentor(String required_skill, int required_skill_level, ArrayList<Integer> index_of_contributors, ArrayList<String> skills_name[], ArrayList<Integer> skills_level[]) {
        
        for(int i=0;i<index_of_contributors.size();i++) {
            if(skills_name[index_of_contributors.get(i)].contains(required_skill)) {
                if(skills_level[index_of_contributors.get(i)].get(skills_name[index_of_contributors.get(i)].indexOf(required_skill)) >= required_skill_level) {
                    return true;
                }
            }
        }
