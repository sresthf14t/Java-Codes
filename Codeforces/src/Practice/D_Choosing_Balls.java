...
public class D_Choosing_Balls {
    ...
    public static void main(String args[]) throws IOException {
        ...
        for(int i=0;i<n;i++) {
            clr[i]=input.scanInt()-1;
        }
        for(int qq=1;qq<=q;qq++) {
            int a=input.scanInt();
            int b=input.scanInt();
            ans.append(solve(n,arr,clr,a,b)+"\n");
        }
        System.out.println(ans);
    }
    public static long solve(int n,long arr[],int clr[],long a,long b) {
        long dp[]=new long[n];
        Arrays.fill(dp, Long.MIN_VALUE);
        long max=Long.MIN_VALUE,sec_max=Long.MIN_VALUE;
        int max_indx=-1,sec_max_indx=-1;
        for(int i=0;i<n;i++) {
            if(dp[clr[i]]!=Long.MIN_VALUE) {
                dp[clr[i]]=Math.max(dp[clr[i]],dp[clr[i]]+a*arr[i]);
            }
            dp[clr[i]]=Math.max(dp[clr[i]],b*arr[i]);
            if(max_indx!=-1 && max_indx!=clr[i]) {
                dp[clr[i]]=Math.max(dp[clr[i]],(b*arr[i])+max);
            }
            if(sec_max_indx!=-1 && sec_max_indx!=clr[i]) {
                dp[clr[i]]=Math.max(dp[clr[i]],(b*arr[i])+sec_max);
            }
            if(dp[clr[i]]>max) {
                if(max_indx==clr[i]) {
                    max=dp[clr[i]];
            ...
            else if(dp[clr[i]]>sec_max && clr[i]!=max_indx) {
                sec_max=dp[clr[i]];
                sec_max_indx=clr[i];
            }
        }
