...
//        Set<Integer> hash_set_books_in_signup_days[] = new HashSet[10];
//        hash_set_books_in_signup_days[0]=new HashSet<Integer>();
//        System.out.println(hash_set_books_in_signup_days[0].isEmpty());
//        System.out.println(hash_set_books_in_signup_days[0].size());
...
//        }
        ...
        Set<Integer> hash_set_books_in_signup_days[] = new HashSet[10];
        for(int i=0;i<10;i++) {
            hash_set_books_in_signup_days[i]=new HashSet<Integer>();
        }
        
        for(int i=0;i<Lib;i++) {
            for(int j=0;j<books_in_lib[i].size();j++) {
                if(!hash_set_books_in_signup_days[no_of_days_sign[i]-1].contains(books_in_lib[i].get(j))) {
                    hash_set_books_in_signup_days[no_of_days_sign[i]-1].add(books_in_lib[i].get(j));
                }
            }
        }
        
        int original_size_of_hash_set_books_in_signup_days[]=new int[10];
        
        for(int i=0;i<10;i++) {
            original_size_of_hash_set_books_in_signup_days[i]=hash_set_books_in_signup_days[i].size();
        }
        ...
        int count_lib=Lib;
        int no_of_days_sign_to_take=1;
        while(Days>0 && count_lib>0) {
            
            int best_lib=find_best_Lib(Lib,Days,b_score , no_of_days_sign, no_of_books_to_scan, books_in_lib, book_is_taken,no_of_books_in_lib,no_of_days_sign_to_take);
//            System.out.println(best_lib);
            if((double)hash_set_books_in_signup_days[no_of_days_sign_to_take].size()/(double)original_size_of_hash_set_books_in_signup_days[no_of_days_sign_to_take]<0.5) {
                no_of_days_sign_to_take+=2;
            }
            System.out.println(no_of_days_sign_to_take+".)"+hash_set_books_in_signup_days[no_of_days_sign_to_take].size());
            if(best_lib==-1) {
                no_of_days_sign_to_take+=2;
                if(no_of_days_sign_to_take>=10) {
                    break;
                }
                continue;
            }
...
//                System.out.print(books_rem_to_send[j]+" ");
                ...
                if(hash_set_books_in_signup_days[no_of_days_sign[best_lib]].contains(books_rem_to_send[j])) {
                    hash_set_books_in_signup_days[no_of_days_sign[best_lib]].remove(books_rem_to_send[j]);
                }
            }
            if(count>0) {
...
//               System.out.println(books);
    ...
    public static int find_best_Lib(int Lib,int Days, int[] b_score ,int[] no_of_days_sign, int[] no_of_books_to_scan, ArrayList<Integer>[] books_in_lib, boolean[] book_is_taken,int[] no_of_books_in_lib,int no_of_days_sign_to_take) {
        int best_lib=-1,best_score=0;
        for(int i=0;i<Lib;i++) {
            if(Lib_is_taken[i]) {
                continue;
            }
            if(no_of_days_sign_to_take==no_of_days_sign[i]) {
