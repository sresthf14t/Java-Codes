...
public class TREEMAX1 {
    ...
    static class seg_tree {
        ...
        seg_tree(int n,int arr[]) {
            seg_tree=new int[8*n];
            create_seg_tree(arr,0,0,n-1);
        }

        //0 index-Left child-(2*i+1) Right Child-(2*i+2)

        void create_seg_tree(int arr[],int vertex,int l,int r) {
            if(l==r) {
        ...
        int max(int vertex,int l,int r,int ql,int qr) {  //ql->query left , qr-> query right l->curr Segmrnt left    r->curr segment right
            if(ql>qr) {
        ...
        void update(int vertex,int l,int r,int pos,int value) {   //pos->Position of the update   value->updates value
            if(l==r) {
    ...
    static int parent[],euler_tour_depth[],euler_tour[],arr[];
    public static void main(String args[]) throws IOException {
        ...
        for(int tt=1;tt<=test;tt++) {
            ...
            Arrays.fill(lst, Integer.MIN_VALUE);            
            
            int brr[]=new int[et.size()];
            Arrays.fill(brr, Integer.MIN_VALUE);
            for(int i=0;i<et.size();i++) {
                if(first[et.get(i)]==Integer.MIN_VALUE) {
                    first[et.get(i)]=i;
                    brr[i]=arr[et.get(i)];
                }
                lst[et.get(i)]=i;
            ...
            for(int i=0;i<q;i++) {
                ...
                if(type==1) {
                    ...
                    st.update(0, 0, brr.length-1, first[u], val);
                }
                else {
                    int u=input.scanInt()-1;
                    int val=st.max(0, 0, brr.length-1, first[u], lst[u]);
                    ans.append(val+"\n");
                }
            }
