...
import java.util.*;
import java.io.*;
public class LITE_Light_Switching {
    static class Scan {
    ...
    static class seg_tree {
        ...
        public void push(int vertex,int l,int r) {
            ...
            int rr=r;
            if(lazy[vertex]%2==1) {
                seg_tree[(2*vertex)+1]=(lr-ll+1)-seg_tree[(2*vertex)+1];
            }
            if(lazy[vertex]%2==1) {
                seg_tree[(2*vertex)+2]=(rr-rl+1)-seg_tree[(2*vertex)+2];
            }
        ...
        public void update(int vertex,int l,int r,int ql,int qr) {
            if(ql>qr) {
                return;
            }
            if(l==ql && r==qr) {
                seg_tree[vertex]=(r-l+1)-seg_tree[vertex];
                lazy[vertex]++;
                return;
            }
            push(vertex,l,r);
            ...
            int mid=(l+r)/2;
            //Left Child
            update((2*vertex)+1,l,mid,ql,Math.min(mid,qr));
            //Right Child
            update((2*vertex)+2,mid+1,r,Math.max(mid+1,ql),qr);
