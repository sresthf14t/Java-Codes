    ...
    public static void main(String args[]) throws IOException {
        ...
        for(int i=0;i<m;i++) {
            ...
            if(!map.containsKey(r[i])) {
                ...
                ArrayList<Integer> tmp=map.get(r[i]);
                tmp.add(l[i]);
            }
            if(!hashset.contains(l[i])) {
                hashset.add(l[i]);
                arrli.add(l[i]);
            }
            if(!hashset.contains(r[i])) {
                hashset.add(r[i]);
                arrli.add(r[i]);
            }
        }
        arrli.sort(null);
        if(arrli.size()==0 || arrli.get(0)!=0) {
            System.out.println(0);
            return;
        }
        if(arrli.size()==0 || arrli.get(arrli.size()-1)!=n) {
            System.out.println(0);
            return;
        }
        for(int i=0;i<arrli.size();i++) {
            indx_map.put(arrli.get(i), i);
        }
        long dp[]=new long[arrli.size()];
        dp[0]=1;
        seg_tree st=new seg_tree(dp.length,dp);
        for(int i=1;i<dp.length;i++) {
            if(!map.containsKey(arrli.get(i))) {
                continue;
            }
            ArrayList<Integer> tmp=map.get(arrli.get(i));
            for(int j=0;j<tmp.size();j++) {
                int lft=indx_map.get(tmp.get(j));
                int rgt=i-1;
                dp[i]+=st.sum(0, 0, dp.length-1, lft, rgt);
                dp[i]%=mod;
            }
            st.update(0, 0, dp.length-1, i, dp[i]);
        }
