...
public class BF_2 {
    ...
    public static void solve(int t) {
        ...
        for(int indx=0;indx<tasks;indx++) {
            ...
            boolean is_pos=true;
            int closest_mount_indx=closest_mount(i);
            int strt_x=m_points_x[closest_mount_indx],strt_y=m_points_y[closest_mount_indx];
...
//                System.out.println(strt_x+" "+strt_y+" "+target_x+" "+target_y+" "+tmp);
            ...
            if(is_pos) {
                ...
                if(time+tmp.length()<=steps) {
                    count++;
                    while(mounting_point_path[closest_mount_indx].length()!=time) {
                        mounting_point_path[closest_mount_indx].append('W');
                    }
...
//        System.out.println();
    ...
    public static void Write_to_File(int count,ArrayList<Integer> tasks_cmpleted[],StringBuilder mounting_point_path[],int t) {
        ...
        for(int j=0;j<tasks_cmpleted.length;j++) {
            ...
            outputstream.println(m_points_x[j]+" "+m_points_y[j]+" "+tasks_cmpleted[j].size()+" "+mounting_point_path[j].length());
            for(int i=0;i<tasks_cmpleted[j].size();i++) {
                outputstream.print(tasks_cmpleted[j].get(i)+" ");
            }
            outputstream.println();
            for(int i=0;i<mounting_point_path[j].length();i++) {
    ...
    public static int closest_mount(int indx) {
        ...
        for(int i=0;i<m_points_x.length;i++) {
            ...
            for(int j=1;j<task_description[indx].size();j+=2) {
                tmp_dist+=Math.abs(m_points_x[i]-task_description[indx].get(j))+Math.abs(m_points_y[i]-task_description[indx].get(j+1));
            }
            if(tmp_dist<dist) {
                dist=tmp_dist;
                dist_indx=i;
