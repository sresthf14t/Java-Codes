...
public class D {
    ...
    static class seg_tree {
        ...
        public int[] min(int vertex,int l,int r,int ql,int qr,int x) {  //ql->query left , qr-> query right l->curr Segmrnt left    r->curr segment right
            if(ql>qr) {
                return new int[]{Integer.MAX_VALUE,-1}; //val indx
            }
            ...
            int min1[]=min((2*vertex)+1,l,mid,ql,Math.min(qr, mid),x);
            int min2[]=min((2*vertex)+2,mid+1,r,Math.max(mid+1,ql),qr,x);
            if(min1[0]<=x) {
                return min1;
            }
            if(min2[0]<=x) {
