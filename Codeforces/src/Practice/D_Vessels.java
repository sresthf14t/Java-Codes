...
public class D_Vessels {
    ...
    static class seg_tree {
        ...
        public int[] max(int vertex,int l,int r,int ql,int qr) {  //ql->query left , qr-> query right l->curr Segmrnt left    r->curr segment right
            ...
            //Left Child
            int ll[]=max((2*vertex)+1,l,mid,ql,Math.min(qr, mid));
            int rr[]=max((2*vertex)+2,mid+1,r,Math.max(mid+1,ql),qr);
            if(ll[0]>=rr[0]) {
                return ll;
            }
            return rr;
    ...
    public static void main(String args[]) throws IOException {
        ...
        for(int q=1;q<=qq;q++) {
            ...
            if(type==1) {
                ...
                int add=input.scanInt();
                if(st.max(0, 0, n-1, indx, n-1)[0]==0) {
                    continue;
                }
                indx=st.max(0, 0, n-1, indx, n-1)[1];
                while(indx<n && add>0) {
                    int rem=arr[indx]-fill[indx];
                    if(rem>add) {
                        fill[indx]+=add;
                        add=0;
                    }
