...
import java.util.*;
import java.io.*;
public class E_Increasing_Frequency {
    static class Scan {
    ...
    public static void main(String args[]) throws IOException {
        ...
        int n=input.scanInt();
        int c=input.scanInt();
        seg_tree st=new seg_tree(500005,new int[500005]);
        int arr[]=new int[n];
        for(int i=0;i<n;i++) {
            arr[i]=input.scanInt();
        }
        int add=0,sub=0,strt=0,end=0,max=0,mstrt=-1,mend=-1;
        for(int i=0;i<n;i++) {
            if(arr[i]==c) {
                sub+=2;
//                continue;
                ...
                st.update(0, 0, 500005, arr[i], st.max(0, 0, 500005, arr[i], arr[i])+1);
            }
            add=st.max(0, 0, 500005, 0, 500005);
            ...
            if(add-sub<=0) {
                for(int j=strt;j<=i;j++) {
                    ...
                    st.update(0, 0, 500005, arr[j], st.max(0, 0, 500005, arr[j], arr[j])-1);
                }
                strt=i+1;
                end=i+1;
                sub=0;
            ...
            System.out.println(i+" "+add+" "+sub+" "+max+" "+strt+" "+end);
        }
        
        System.out.println(mstrt+" "+mend+" "+max);
