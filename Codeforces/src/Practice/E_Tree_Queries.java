...
public class E_Tree_Queries {
    ...
    static int euler_tour_depth[],euler_tour[];
...
//            System.out.println();
            ...
            for(int j=0;j<k;j++) {
                int indx=j;
                while(indx<k && depth[arr[j]]==depth[arr[indx]]) {
                    indx++;
                }
                indx--;
//                System.out.println(j+" "+indx);
                for(int tmp=j;tmp<=indx;tmp++) {
                    if(parent[arr[tmp]]!=parent[arr[j]]) {
                        is_pos=false;
                        break;
                    }
                }
                ...
                if(prev_par!=-1) {
                    int u=prev_par;
                    int v=Math.max(0,parent[arr[j]]);
                    int l=Math.min(first[u],first[v]);
                    int r=Math.max(first[u],first[v]);
                    indx=st.min(0, 0, euler_tour_depth.length-1, l, r)[1];
//                    System.out.println((arr[j]+1)+" "+(prev_par+1)+" "+(euler_tour[indx]+1));
                    if(euler_tour[indx]!=Math.max(0,prev_par)) {
                        is_pos=false;
                    }
                }
                if(!is_pos) {
    ...
    public static void DFS_et(int root,int par,int dep) {
        parent[root]=par;
        depth[root]=dep;
        et.add(root);
        etd.add(dep);
        for(int i=0;i<adj_lst[root].size();i++) {
            ...
            DFS_et(adj_lst[root].get(i),root,dep+1);
            et.add(root);
            etd.add(dep);
        }
    }
    ...
    public static void merge(int arr[],int l1,int r1,int l2,int r2) {
        ...
        for(int i=0;i<tmp.length;i++) {
            ...
            if(depth[arr[indx1]]<depth[arr[indx2]]) {
                tmp[i]=arr[indx1];
                indx1++;
                continue;
            }
