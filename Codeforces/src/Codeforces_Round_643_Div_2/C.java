...
public class C {
    ...
    public static long solve(long a,long b,long c,long d) {
        ...
        for(long x=a;x<=b;x++) {
            long min_y=b;
            long max_y=c;
            long min_z=x+min_y-1;
            long max_z=x+max_y-1;
            long num=c-b+1;
            if(max_z>d) {
                if(max_z-d>num) {
                    count+=(num)*(d-c+1);
                ...
                count+=sum_ap(d,-1,num);
            }
            else {
                if(max_z-c+1>num) {
                    count+=sum_ap(max_z,-1,num);
                }
                else {
                    count+=sum_ap(max_z,-1,max_z-c+1);
                }
            }
        }
        return count;
    }
    public static long sum_ap(long a,long d,long n) {
        long sum=(2*a)+(n-1)*d;
        sum*=n;
        sum/=2;
        return sum;
