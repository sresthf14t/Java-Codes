...
public class SADSALE {
    ...
    static ArrayList<Integer> adj_lst[],wei[],et,etd;
    static long psum[];
...
//                System.out.println(u+" "+v+" "+lca);
                if(lca==u) {
                    long fin=psum[v]-psum[u];
                    ans.append(fin+"\n");
                }
                else if(lca==v) {
                    long fin=psum[u]-psum[v];
                    ans.append(fin+"\n");
                }
                else {
                    long fin=psum[u]+psum[v]-2*(psum[lca]);
                    ans.append(fin+"\n");
                }
    ...
    public static void DFS(int root,int par,int dep,long sum) {
        psum[root]=sum;
        parent[root]=par;
        et.add(root);
        etd.add(dep);
        for(int i=0;i<adj_lst[root].size();i++) {
            ...
            DFS(adj_lst[root].get(i),root,dep+1,sum+wei[root].get(i));
            et.add(root);
            etd.add(dep);
        }
    }
